...
const UpdatePostPublish = () => {
  const navigate = useNavigate()
  const location = useLocation()
  const { title, categoryId, content, id } = location.state || {}
  ...
  const handlePublish = async () => {
    ...
    console.log("postData before : ", title, categoryId, content);
    ...
    try {
      const response = await axios.put(
          `http://localhost:8080/c/post/update/${id}`,
          postData,
          {
            headers: {
...
export default UpdatePostPublish
